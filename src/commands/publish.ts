...
import { readCloudConfig } from '../shared/config.js';
 ...
 * 2、调用 fetchConfig 方法获取 target、bucket、region 等配置
 * 3、根据第 2 步的 target 读取 ~/.bfun/config.json 中对应的 accessKey、secretKey
 * 4、调用 uploadDir 函数上传第 1 步中指定目录下的 dist 目录到对应的云端目录
 * 5、调用 updateConfig 方法更新配置
 * 5.1 上传成功后，将新的版本号更新到读取时的 package.json 中
 */
export default class Publish extends Command {
  static override args = {
    dir: Args.string({
      default: '.',
      description: '要发布的项目目录，默认为当前执行命令的目录',
    }),
  };
  static override description = '发布项目到云端';
  ...
  static override flags = {
    ...
    force: Flags.boolean({ char: 'f', description: '强制发布，不检查版本' }),
  };
  ...
  public async run(): Promise<void> {
    ...
    logger.success(`最新发布版本: ${newVersion}`);

    // 步骤 2：调用 fetchConfig 获取 target、bucket、region 等配置
    logger.info('正在获取发布配置 ...');
    const { bucket, prefix, region, target } = await fetchConfig({
      appId,
      token,
    });
    logger.success(
      `配置获取成功 - target: ${target}, bucket: ${bucket}, region: ${region}`,
    );

    // 步骤 3：根据 target 读取 ~/.bfun/config.json 中对应的 accessKey、secretKey
    const { accessKey, secretKey } = readCloudConfig(target);

    // 步骤 4：调用 uploadDir 上传 dist 目录到对应的云端目录
    ...
    const cloudDir = `${prefix}/${appId}/${newVersion}`.replaceAll(
      /\/\/+/g,
      '/',
    );
    ...
    logger.success(`上传完成 - 版本 ${newVersion} 已发布到 ${target}`);

    // 步骤 5：调用 updateConfig 更新配置
    logger.info('正在更新配置 ...');
    const result = await updateConfig({
      appId,
      iterationData: { bucket, prefix, region, target, version: newVersion },
    ...
    if (result.success) {
      ...
      logger.warn('配置更新失败，但发布已完成');
    }

    // 步骤 5.1：上传成功后，将新的版本号更新到读取时的 package.json 中
    this.updatePackageVersion(projectDir, newVersion);

    logger.newline();
    logger.success(`发布完成! appId: ${appId}, version: ${newVersion}`);
  }
